...
```javascript
  ...
  // 1. Fetch current retry state
  const retryCheck = await pool.query(
    'SELECT retry_count, max_retries FROM jobs WHERE id = $1', [jobId]
  );
  const { retry_count, max_retries } = retryCheck.rows[0];

  if (retry_count < max_retries) {
    // 2. Transition back to Queued (Retry)
    await pool.query(
      `UPDATE jobs SET retry_count = $1, status = 'queued', error = $2 WHERE id = $3`,
      [retry_count + 1, error.message, jobId]
    );
    // Push back to Redis after delay...
  } else {
    // 3. Transition to Failed (Dead-Letter)
    await pool.query(
...
**Answer:** "This is a classic distributed systems problem known as the 'stuck in processing' issue. Currently, the job remains in PostgreSQL with a status of `processing`, but it is no longer in Redis. In a production environment, I would solve this by running a cron job every few minutes that queries Postgres for jobs stuck in `processing` for more than X minutes (a timeout threshold). The cron job would automatically transition them back to `queued` so another worker can pick them up."
