...
**Exact Script:**
"Absolutely. In a high-level system design, relying on a single database for a queue forces you to choose between speed and safety. I bypassed this by separating state from speed:

*   **PostgreSQL (The Brain):** This is my durable source of truth. It guarantees no job is ever lost because data is safely written to disk.
*   **Redis (The Reflexes):** Postgres is terrible for polling (it crushes the CPU). So, I used Redis as an in-memory dispatcher for blazing-fast speed.

**Here is the exact step-by-step flow in my code:**
1.  **Ingestion (`app.js`):** To prevent data loss, the API does a dual-write. It first runs an `INSERT` into Postgres to permanently save the job, and *only then* does it use `LPUSH` to put the job ID into a Redis priority queue.
2.  **Consumption (`worker.js`):** Workers don't constantly poll the database. Instead, they use Redis's `BRPOP` command. This completely blocks the connection at 0% CPU usage and wakes the worker up the exact millisecond a job arrives. The worker then runs an `UPDATE` in Postgres to claim it.
3.  **Retries (`ZADD`):** If a job fails, I don't use tricky Postgres schedulers. I calculate an exponential backoff delay and push the job into a Redis Sorted Set (`DELAYED_QUEUE`). A lightweight loop checks this set and re-enqueues jobs when their time is up.
4.  **Self-Healing (The Safety Net):** If a Docker container crashes, Redis and Postgres might drift out of sync. My `worker.js` runs a periodic maintenance loop (like `recoverStalledProcessingJobs`) to scan Postgres for jobs that timed out and automatically re-enqueues them so nothing is ever permanently stuck.
5.  **Real-Time UI:** As jobs move through this pipeline, the backend uses Redis Pub/Sub to instantly emit WebSocket events, updating the React dashboard in real time."

**💡 Interview Flex (The Mic Drop):**
"By combining Redis's atomic operations with PostgreSQL row-level locks (using a UUID `processing_token` when claiming a job), I mathematically eliminated Race Conditions. Even if we spun up 50 concurrent worker nodes right now, they could never accidentally claim and execute the same job twice."
