...
**Exact Script:**
"QueueFlow is designed for horizontal scalability because the API and workers are stateless, while job state is stored in shared infrastructure. At 10,000 RPS, I would first horizontally scale the worker fleet using Kubernetes. An HPA could scale workers based on metrics such as CPU utilization and, more importantly for a queue system, queue depth.

With many workers operating concurrently, I need to prevent duplicate job ownership. Redis's atomic queue operations ensure a job is popped by one consumer, while PostgreSQL provides a durable state transition using a conditional atomic UPDATE such as `WHERE status = 'queued'`. This gives me a second layer of concurrency protection.

The next bottleneck is PostgreSQL connections. If hundreds of workers each maintain their own connection pools, they can exhaust PostgreSQL's connection capacity. I would place PgBouncer in front of PostgreSQL to pool and multiplex connections.

If database throughput itself becomes the bottleneck, I would scale the data layer horizontally using techniques such as sharding, ideally using a high-cardinality key like a hash of `jobId` to distribute load evenly.

So the scaling strategy is: horizontally scale stateless workers, guarantee safe concurrent job ownership, pool database connections, and finally distribute database workload when a single database reaches its throughput ceiling."
