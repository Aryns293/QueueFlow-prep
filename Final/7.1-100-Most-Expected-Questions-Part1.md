...
<details>
  ...
  **1. First: What does 10,000 RPS actually mean?**
  ...
  Client 3 ┤
  Client 4 ┤
  ...      ├─► API Servers ──► Redis ──► Workers ──► PostgreSQL
  Client N ┘
  ```
  ...
  **2. Horizontal Scaling**
  What is horizontal scaling? There are two basic ways to scale a system.
  ...
  **3. Stateless Design**
  This is a very important interview concept. Your answer says: `"The API and worker nodes are stateless."`
  This means the server doesn't depend on its own local memory to remember important application state. 
  For example, imagine Worker 1 processes `Job #123`.
  You don't want:
  ```
  Worker 1's RAM:
    Job #123
    ...
    Job #789
  ```
  because if `Worker 1 💥` crashes, that information disappears.
  ...
  ```
  The important state lives in shared infrastructure:
  ...
  **4. Kubernetes**
  Now the question becomes: Who creates and manages all these workers? That's where **Kubernetes** comes in.
  ...
  Worker 500
  ```
  That would be terrible. Kubernetes manages these containers for you.
  ```
         Kubernetes
             │
     ┌───────┼───────┐
  ...
  Worker Pod Worker Pod Worker Pod
    ...
    Job     Job     Job
  ```
  A Kubernetes **Pod** is the basic deployable unit, effectively a running instance of your worker application.
  ...
  *   Massive traffic:
  ```
  Redis queue depth = 100,000
           ↓
  500 worker pods
  ```

  **6. What can HPA monitor?**
  *   **CPU utilization:** e.g., `CPU = 90%` → `Workers are overloaded → add more pods.`
  *   **Queue depth:** This is often more meaningful for a queue system.
  ```
  Redis:
  High Priority:  100 jobs
  Normal:       5,000 jobs
  Low:          2,000 jobs
  Total = 7,100 waiting jobs
  ```
  If queue depth continuously increases, your workers aren't processing jobs fast enough:
  ```
  Queue depth ↑
       ↓
  More workers
  ...
  **7. What does "500 workers" actually mean?**
  Don't present 500 as a magic number. The correct engineering approach is to calculate it: `Determine how much work one worker can process, then calculate the required worker count.`
  Suppose one worker can process: `20 jobs/sec`
  Then theoretically: 
  ```
  10,000 jobs/sec ÷ 20 jobs/sec = 500 workers
  ```
  ...
  **8. The Race Condition Problem**
  ...
  Worker 500
  ```
  All workers are trying to obtain jobs. You don't want:
  ```
  Worker 1 → Job #123
  Worker 2 → Job #123
  ```
  because then two workers process the same job.

  **9. Race Condition**
  A race condition happens when multiple processes access/change shared state concurrently and the final result depends on timing.
  Imagine: `Job status = queued`
  *   Worker 1 checks: `Is Job #123 queued? YES`
  *   Worker 2 checks: `Is Job #123 queued? YES`
  Now both think they can process it.

  **10. Redis BRPOP**
  Your architecture uses Redis lists as queues. `BRPOP` = Blocking Right Pop. It means: `"Remove and return an item from the right side of this Redis list. If empty, wait."`
  The pop operation is **atomic**.
  Suppose: `Queue: [Job A, Job B, Job C]`
  Two workers simultaneously request a job. Redis guarantees that one operation happens first:
  ```
  Worker 1 → Job C
  Worker 2 → Job B
  ```

  **11. What does "Redis is single-threaded" mean?**
  Redis processes commands sequentially through its command execution path.
  Think:
  ```
  Request A
      ↓
  ...
  Request C
  ```
  However, don't say `"Redis guarantees exactly-once processing."` A worker can crash *after* receiving the job from Redis. That's why PostgreSQL is important.

  **12. PostgreSQL as the second safety layer**
  ```sql
  UPDATE jobs SET status = 'processing', processing_token = $2 WHERE id = $1 AND status = 'queued';
  ```
  Suppose: `Job #123 status = queued`.
  Worker 1 executes the update. The database changes: `queued → processing`.
  Now suppose Worker 2 somehow tries the same thing. The job is now `processing`, therefore:
  ```sql
  WHERE status = 'queued'
  ```
  is false. So Worker 2 updates **0 rows**.

  **13. Why is this called an Atomic Operation?**
  It is treated as one indivisible database operation.
  You don't want this:
  ```
  1. SELECT status
  2. Check status
  3. UPDATE status
  ```
  because two workers could perform step 1 before either performs step 3. The atomic `UPDATE` handles concurrency correctly.

  **14. Row-Level Locking**
  PostgreSQL coordinates concurrent modifications to the same row.
  Conceptually:
  ```
  Worker 1 ──┐
             ├─► PostgreSQL
  Worker 2 ──┘       │
                 Job #123
  ```
  It ensures conflicting updates don't both independently succeed against the same old state.

  **15. Processing Token**
  Your query sets `processing_token = $2`. This acts as an **ownership token**.
  Think:
  ```
  Job #123
  status = processing
  processing_token = abc-xyz
  ```
  Worker owns `abc-xyz`. Later, it can prove `"I am the worker that owns this processing attempt."` Useful for retries and crashing workers.

  **16. The Database Bottleneck**
  Assume `10,000 jobs/sec`.
  A single job may cause multiple database operations:
  ```
  INSERT job
      ↓
  UPDATE processing
  ...
  UPDATE completed
  ```
  That's approximately `3 DB operations/job`. Therefore:
  ```
  10,000 × 3 = 30,000 database operations/sec
  ```
  This is why the database becomes the bottleneck.

  **17. Important distinction: RPS vs DB QPS**
  ...
  *   **QPS (Queries Per Second):** How many database queries execute (`30,000 QPS`).
  They are not necessarily equal. One HTTP request might generate:
  ```
  1 API request
      ↓
  5 DB queries
  ...
  **18. Connection Exhaustion**
  Now imagine: `500 worker pods`
  Suppose each worker opens: `20 PostgreSQL connections`. That's:
  ```
  500 × 20 = 10,000 connections
  ```
  Too many connections create memory and CPU overhead. Eventually:
  ```
  PostgreSQL
      ↓
  ...
  **19. PgBouncer**
  Think of PgBouncer as a **connection traffic manager**. 
  Without PgBouncer:
  ```
  Worker 1 ──────► PostgreSQL
  Worker 2 ──────► PostgreSQL
  ...
  Worker 500 ────► PostgreSQL
  ```
  With PgBouncer:
  ```
  Worker 1 ┐
  ...
  **20. Connection Pooling**
  Suppose you have `5,000 application connections` but PostgreSQL only needs `200 actual database connections`.
  Conceptually:
  ```
  5,000 clients
        ↓
  ...
  **21. Why PgBouncer doesn't magically increase DB capacity**
  PgBouncer solves `Too many connections`. It does **not** solve `PostgreSQL can't physically process 30,000 writes/sec`.
  ```
  After PgBouncer:
  Connection problem ✅
  But:
  Database CPU / disk I/O
        ↓
       100%
        ↓
  Still bottlenecked ❌
  ...
  **22. Database Sharding**
  Sharding = distributing data across multiple independent database nodes.
  Instead of:
  ```
        PostgreSQL
        /   |   \
  ...
  **23. How does sharding decide where a job goes?**
  A common strategy is **hash-based sharding**. For example: `hash(jobId) % 4`.
  Suppose:
  ```
  hash(Job123) % 4 = 0  → Shard 0
  hash(Job456) % 4 = 2  → Shard 2
  hash(Job789) % 4 = 1  → Shard 1
  ```
  This spreads the workload.

  **24. Why does sharding improve throughput?**
  Imagine one PostgreSQL server can handle `10,000 writes/sec`.
  If you have three independent shards:
  ```
  Shard 1 → 10k writes/sec
  Shard 2 → 10k writes/sec
  Shard 3 → 10k writes/sec
  ```
  Theoretical aggregate: `≈ 30,000 writes/sec`.

  **25. One correction: Don't shard by priority**
  If most jobs are high priority:
  ```
  Shard 1 → 🔥🔥🔥🔥🔥
  Shard 2 → 😐
  Shard 3 → 😐
  ```
  You've created a **hot shard**. Hashing by a high-cardinality identifier like `jobId` distributes load evenly.
  ...
  **26. Putting Everything Together**
  ...
  flowchart TD
      ...
      PgB --> S3[(PG Shard 3)]
  ```
  And Kubernetes sits around the worker/API infrastructure:
  ```
         Kubernetes
             │
     ┌───────┼───────┐
  ...
  **27. The Four Scaling Problems**
  ...
  Horizontal scaling
    ↓
  Need safe concurrency
    ↓
  Atomic queue + DB state transition
    ↓
  Many workers create many DB connections
    ↓
  PgBouncer
    ↓
  Database itself reaches throughput limits
    ↓
  Sharding / database scaling
  ```

  **28. One More Important Concept: Backpressure**
  Suppose traffic suddenly becomes `10,000 RPS` but workers can only process `8,000 jobs/sec`.
  ```
  Incoming:   10,000/sec
  Processing:  8,000/sec
                   ↓
  Queue grows by 2,000/sec
  ```
  That's okay **if the queue is designed to absorb the burst**. The queue acts as a **buffer**:
  ```
  Traffic spike
      ↓
  Redis queue grows
  ...
  ```
  This is a fundamental reason queue-based architectures scale well.
