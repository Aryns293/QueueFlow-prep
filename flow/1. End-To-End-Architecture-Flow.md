...
graph TD
    ...
    F --> G["ZADD jobQueue:delayed (Backoff Delay)"]
...
sequenceDiagram
    ...
    participant ZSET as Redis (ZSET: jobQueue:delayed)
    ...
    Worker->>ZSET: ZADD jobQueue:delayed score payload
    
    Note over Cron: Time (delay) passes...
    
    Cron->>ZSET: ZRANGEBYSCORE jobQueue:delayed 0 current_timestamp
