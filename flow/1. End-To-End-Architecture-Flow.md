...
### Theory: Why Two Databases?
A common interview question is: *"Why use both Postgres and Redis?"*
- **Redis** is single-threaded and strictly in-memory. It can push and pop jobs via `LPUSH`/`BRPOP` in microseconds, handling massive throughput. However, if the server loses power, the RAM is wiped and the queue is lost.
- **PostgreSQL** writes to physical disk and provides ACID (Atomicity, Consistency, Isolation, Durability) guarantees.
- **The Synergy**: We use Postgres to store the *state* of the job. We use Redis to signal *which* jobs are ready. If Redis crashes, our queue is temporarily gone, but we can instantly rebuild it by asking Postgres: `"Give me all jobs where status = 'queued'"`.
