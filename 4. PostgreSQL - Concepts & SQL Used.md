...
```
* **Why it's important:** If two workers run this query on the same `id` simultaneously, Postgres locks the row. The first one gets the data returned. The second gets 0 rows back.
...
To fix this, we created **Indexes** on `status` and `created_at`. An index is like the index at the back of a textbook—Postgres instantly looks up where the 'queued' jobs are and jumps straight to them, turning an $O(N)$ scan into an $O(log N)$ tree traversal.
